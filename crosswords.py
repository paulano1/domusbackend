...
resp = requests.get("https://api.dictionaryapi.dev/api/v2/entries/en/" + "ord")
...
#print(resp["meanings"][1]["definitions"]["definition"])'''
...
class Crossword(object):
    ...
    def word_find(self): # return solution grid
        ...
        for r in range(self.rows):
            for c in self.grid[r]:
                if c == self.empty:
                    outStr += '%s ' % stringLower[random.randint(0,len(stringLower)-1)]
                else:
                    outStr += '%s ' % c
            outStr += '\n'
        return outStr
    ...
    def legend(self): # must order first
        ...
        for word in self.current_word_list:
            ...
            self.data[word.down_across()][word.number] = temp
